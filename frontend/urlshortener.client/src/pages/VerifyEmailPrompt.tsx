...
export function VerifyEmailPrompt() {
  ...
  useEffect(() => {
    ...
    apiClient.post(`/api/auth/send-verification-code-for/${state.userId}`)
      .then(() => setMessage(`Code sent to ${state.email}`))
      .catch(() => {});
  }, []);
  ...
  const handleResend = async () => {
    ...
    setMessage(null);
    try {
      await apiClient.post(`/api/auth/send-verification-code-for/${state.userId}`);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      await apiClient.post(`/api/auth/verify-email-for/${state.userId}`, { code });
      await refreshUser();
      navigate('/dashboard');
    } catch {
    ...
    <AuthLayout>
      <Card className="w-full max-w-md text-center" shadowColor="pink">
        <div className="w-16 h-16 rounded-full bg-tertiary/20 flex items-center justify-center mx-auto mb-4">
          <MailWarning size={28} className="text-tertiary" />
        </div>
        <h1 className="text-2xl mb-1">Verify your email to continue</h1>
        ...
        <p className="text-center font-body text-mutedForeground mt-6">
